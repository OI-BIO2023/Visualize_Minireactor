    ...
    <article className="panel reactor-card">
      ...
      <div className="subgrid">
        <div className="subpanel">
          ...
          <p>Innenraum: <strong>{value(data?.[`T_Innenraum_${reactor}`])} °C</strong></p>
          <p>Feuchte oben/unten: <strong>{value(data?.[`HUM_oben_${reactor}`])} / {value(data?.[`HUM_unten_${reactor}`])} %</strong></p>
        </div>
        <div className="subpanel">
          <h3>Wärmeentzug</h3>
          <p>VL/RL: <strong>{value(data?.[`T_VL_${reactor}`])} / {value(data?.[`T_RL_${reactor}`])} °C</strong></p>
        ...
        <div className="subpanel">
          ...
          <div className="chip-row">
            <span className={`status-badge ${Number(data?.[`Q_IRR_${reactor}`] ?? 0) > 0 ? 'success' : ''}`}>Bewässerung</span>
            <span className={`status-badge ${(Number(data?.[`V${reactor.slice(1)}_1_V_VL`]) ?? 0) > 0 ? 'success' : 'muted'}`}>Wärmeentzug</span>
            <span className={`status-badge ${(Number(data?.[`V${reactor.slice(1)}_3_V_AER`]) ?? 0) > 0 ? 'success' : 'muted'}`}>Belüftung</span>
