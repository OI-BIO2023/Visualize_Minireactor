    ...
    <main className="page">
      <header className="hero">
        <div>
          <h1>Historie</h1>
          <p className="muted">
            Batch {batch.batch_id} für {reactor}
          </p>
        </div>
      ...
      <section className="panel">
        ...
        <div className="metric-grid metric-grid-wide">
          ...
          <article className="metric-card">
            ...
            <strong>{metrics.maxTemperature == null ? '–' : metrics.maxTemperature.toFixed(1)}</strong>
            <span className="metric-unit">°C</span>
          </article>
          <article className="metric-card">
            <span className="metric-label">Mittel</span>
            <strong>{metrics.meanTemperature == null ? '–' : metrics.meanTemperature.toFixed(1)}</strong>
            <span className="metric-unit">°C</span>
          </article>
          <article className="metric-card">
            <span className="metric-label">Gradient</span>
            <strong>{Number.isFinite(metrics.gradient) ? metrics.gradient.toFixed(1) : '–'}</strong>
            <span className="metric-unit">°C</span>
          </article>
          <article className="metric-card">
            <span className="metric-label">Frischwasseranteil</span>
            <strong>{metrics.waterShare == null ? '–' : (metrics.waterShare * 100).toFixed(1)}</strong>
            <span className="metric-unit">%</span>
          </article>
          <article className="metric-card">
            <span className="metric-label">Bewässerung</span>
            <strong>{metrics.irrigationHours?.toFixed(1) ?? '–'}</strong>
            <span className="metric-unit">h</span>
          </article>
          <article className="metric-card">
            <span className="metric-label">Wärmeentzug</span>
            <strong>{metrics.heatHours?.toFixed(1) ?? '–'}</strong>
            <span className="metric-unit">h</span>
          </article>
          <article className="metric-card">
            <span className="metric-label">Energie</span>
            <strong>{metrics.heatEnergy == null ? '–' : metrics.heatEnergy.toFixed(2)}</strong>
      ...
      <section className="panel">
        ...
        <p className="muted">Die Auswertung ist um Batch-Zeitfenster herum aufgebaut und kann für spätere ML-Exporte direkt wiederverwendet werden.</p>
      </section>
      <section className="panel">
        <h2>Zeitraum</h2>
        <p className="muted">
